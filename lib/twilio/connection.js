...
Connection.prototype.accept = function(handler) {
    ...
    function connect_() {
        ...
        if (self._direction === "INCOMING") {
            ...
            self.mediaStream.makeOutgoingCall(self.pstream.token, params, self.outboundConnectionId, self.options.rtcConstraints, self.options.iceServers, onRemoteAnswer);
        }
...
Connection.prototype._onRinging = function(payload) {
    ...
    if (this.options.enableRingingState) {
        ...
        this._publisher.info("connection", "outgoing-ringing", {
